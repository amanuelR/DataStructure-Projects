...
void Player::update() {
	...
	// Point initial (p.getX(), p.getY()-1);

	//N :(p.getX(), p.getY()-1) if(this point has a state free, add to stack)
	//E if(this point has a state free, add to stack)
	//S if(this point has a state free, add to stack)
	//W
	//setState(State::LOOKING);
	Point North, East, South, West;
	Point p1 = getTargetPoint();
	if (m_look.empty()) {
			setState(State::STUCK);
			return;
	}
	if (p1 == getAquarium()->getEndPoint()) {
	...
	if (getState() == State::LOOKING)
	{
		...
		m_look.pop();
		//Check West slot
		West.set(p1.getX() + 1, p1.getY());
		if (getAquarium()->isCellOpen(West) && !discovered(West)) {
			curr = West;
			m_discovered.push_back(West);
			m_look.push(West);
		}
		//Check East slot
		East.set(p1.getX() - 1, p1.getY());
		if (getAquarium()->isCellOpen(East) && !discovered(East)) {
			curr = East;
			m_discovered.push_back(East);
			m_look.push(East);
		}
		//Check North slot
		North.set(p1.getX(), p1.getY() - 1);
		if (getAquarium()->isCellOpen(North) && !discovered(North)) {
			curr = North;
			m_discovered.push_back(North);
			m_look.push(North);
		}
		//Check South slot
		South.set(p1.getX(), p1.getY() + 1);
		if (getAquarium()->isCellOpen(South) && !discovered(South)) {
			curr = South;
			m_discovered.push_back(South);
			m_look.push(South);
		}
	}
		/*if (m_look.peek() != West && m_look.peek() != East && m_look.peek() != North && m_look.peek() != South)
		{
	...
	{
		West.set(getPosition().getX() + 1, getPosition().getY());
		East.set(getPosition().getX()-1, getPosition().getY());
		North.set(getPosition().getX(), getPosition().getY() - 1);
		South.set(getPosition().getX(), getPosition().getY() + 1);

		if (!m_btStack.empty())
		{
			Point XY = m_btStack.peek();
			m_btStack.pop();
			setPosition(XY);
		}
		if (South == m_look.peek())
			m_btStack.push(South);
		if (North == m_look.peek())
			m_btStack.push(North);
		if (East == m_look.peek())
			m_btStack.push(East);
		if (West == m_look.peek())
			m_btStack.push(West);
		Point curr = m_btStack.peek();
		m_btStack.pop();
		setPosition(curr);
